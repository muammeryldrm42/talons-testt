...
async function withPage<T>(fn: (page: any) => Promise<T>): Promise<T> {
  ...
  const context = await browser.newContext({
    userAgent: "Mozilla/5.0 (Open Talons Bot)",
...
export default definePlugin({
  name: "@open-talons/plugin-browser",
  version: "1.0.0",
  ...
  agents: [defineAgent({
    ...
    icon: "🌐",
    name: "Browser Agent",
    systemPrompt: `🌐 Open Talons Browser Agent — automate web pages with Playwright.
- browser_scrape: extract text/HTML (use CSS selectors for precision)
- browser_screenshot: capture pages as PNG
- browser_extract_links: get all links
- browser_click_extract: click + read
- browser_fill_form: form automation
- browser_eval_js: advanced JS in page context

Use respectfully. Don't bypass rate limits, captcha, or auth. For scraping high-volume, prefer official APIs.`,
    allowedTools: ["browser_scrape", "browser_screenshot", "browser_extract_links", "browser_click_extract", "browser_fill_form", "browser_eval_js"],
  })],
});
