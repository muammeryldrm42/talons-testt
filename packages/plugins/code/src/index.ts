...
async function runCode(language: "python" | "node" | "typescript", code: string, timeoutMs = 30000): Promise<ToolResult> {
  ...
  const tmpFile = path.join(tmpDir, `open-talons-${id}.${ext}`);
...
export default definePlugin({
  ...
  agents: [defineAgent({
    ...
    systemPrompt: `🔬 Open Talons Code Interpreter — write and execute code in Python, JavaScript, or TypeScript.
- Use run_python for data analysis, calculations, scraping
- Use run_javascript / run_typescript for web/JSON work
- Install packages with install_python_package / install_node_package
- Always show output cleanly, handle errors
- Code runs in /tmp with 30s default timeout — use timeout_ms for longer
- Each execution requires user permission`,
    allowedTools: ["run_python", "run_javascript", "run_typescript", "install_python_package", "install_node_package"],
  })],
});
