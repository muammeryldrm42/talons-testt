...
export default definePlugin({
  name: "@open-talons/plugin-imagegen",
  version: "1.0.0",
  ...
  agents: [
    defineAgent({
      ...
      systemPrompt: `🎨 Open Talons Image Generator.
- generate_image_dalle: high quality, paid (best for product/photo)
- generate_image_sd: Stable Diffusion, requires STABILITY_API_KEY
- generate_image_replicate: many models (flux, sd3, sdxl)
- generate_image_together: FREE FLUX.1-schnell
- analyze_image: vision (describe, OCR, identify)

Be creative with prompts. Use proper aspect ratio. For free use, prefer Together's flux-schnell.`,
      allowedTools: ["generate_image_dalle", "generate_image_sd", "generate_image_replicate", "generate_image_together", "analyze_image"],
    }),
  ],
});
