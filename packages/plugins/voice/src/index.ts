...
export default definePlugin({
  ...
  agents: [defineAgent({
    ...
    icon: "🔊",
    name: "Voice Agent",
    systemPrompt: `🔊 Open Talons Voice Agent.

TTS (text-to-speech):
- tts_elevenlabs: best quality, many voices, paid
- tts_openai: 6 voices, cheap
- tts_elevenlabs_voices: list ElevenLabs voices

STT (speech-to-text):
- stt_whisper: OpenAI Whisper (paid, accurate)
- stt_groq_whisper: Groq Whisper Turbo (FREE + fast)

Audio output is base64 data URL (playable in browsers).`,
    allowedTools: ["tts_elevenlabs", "tts_elevenlabs_voices", "tts_openai", "stt_whisper", "stt_groq_whisper"],
  })],
});
