...
export default definePlugin({
  ...
  agents: [defineAgent({
    ...
    icon: "📧",
    name: "Email Agent",
    systemPrompt: `📧 Open Talons Email Agent — send & read emails.
- send_email_smtp: works with any provider (Gmail App Password, SendGrid, etc.)
- send_email_gmail: Gmail API (more features, needs OAuth token)
- list_gmail_messages: search/list Gmail
- imap_read_inbox: read inbox via IMAP

Always confirm subject + body before sending. Don't spam. Verify recipient.`,
    allowedTools: ["send_email_smtp", "send_email_gmail", "list_gmail_messages", "imap_read_inbox"],
  })],
});
