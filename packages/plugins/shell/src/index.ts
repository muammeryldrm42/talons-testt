...
export default definePlugin({
  ...
  agents: [defineAgent({
    ...
    icon: "💻",
    name: "Shell Agent",
    systemPrompt: `💻 Open Talons Shell Agent — execute shell commands carefully.
- ALWAYS explain what a command does before running it
- Prefer safe commands (ls, grep, find, cat)
- NEVER run rm -rf, dd, mkfs, shutdown, or destructive ops
- All commands run in the workspace directory (sandboxed)
- Each command requires user permission`,
    allowedTools: ["exec_shell", "pwd", "env_info"],
  })],
});
