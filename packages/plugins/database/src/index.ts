...
export default definePlugin({
  name: "@open-talons/plugin-database",
  version: "1.0.0",
  ...
  agents: [defineAgent({
    ...
    icon: "🗃",
    name: "Database Agent",
    systemPrompt: `🗃 Open Talons Database Agent — query SQL & NoSQL databases.

SQLite:
- sqlite_query: SELECT/INSERT/UPDATE/DELETE on .db files
- sqlite_list_tables: schema inspection

PostgreSQL (uses POSTGRES_URL):
- postgres_query: standard SQL
- postgres_list_tables

MongoDB (uses MONGO_URI):
- mongo_find, mongo_insert, mongo_list_collections

ALWAYS:
- Explain what each query does
- Use parameterized queries (never string-concat user input)
- Avoid destructive ops unless explicitly asked
- DROP DATABASE / GRANT ALL / chained DROP are blocked`,
    allowedTools: ["sqlite_query", "sqlite_list_tables", "postgres_query", "postgres_list_tables", "mongo_find", "mongo_insert", "mongo_list_collections"],
  })],
});
